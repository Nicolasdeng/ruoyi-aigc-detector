  ...
  <view class="container">
    ...
    <view class="header">
      <view class="header-content">
        <text class="title">AI音频检测</text>
        <text class="subtitle">智能识别AI生成音频</text>
      </view>
    </view>
    ...
    <view class="detect-area">
      <!-- 上传音频区域 -->
      <view class="upload-section" v-if="!audioUrl">
        <view class="upload-box" @tap="chooseAudio">
          <text class="upload-icon">🎵</text>
          <text class="upload-text">点击上传音频</text>
          <text class="upload-tip">支持MP3、WAV格式，最大50MB</text>
        </view>
      </view>

      <!-- 音频预览区域 -->
      <view class="preview-section" v-if="audioUrl">
        <view class="audio-card">
          <view class="audio-info">
            <text class="audio-icon">🎵</text>
            <view class="audio-details">
              <text class="audio-name">音频文件</text>
              <text class="audio-size">{{ audioSize }}</text>
            </view>
          </view>
          <button class="reupload-btn-small" @tap="chooseAudio">重新上传</button>
        </view>
      </view>

      <!-- 检测按钮 -->
      <view class="action-section" v-if="audioUrl && !detecting">
      ...
      <view class="detecting-section" v-if="detecting">
        <view class="loading-animation">
          ...
          <text class="loading-text">AI音频分析中...</text>
          <text class="loading-desc">正在分析音频特征</text>
        </view>
      </view>
      ...
      <view class="result-section" v-if="detectionResult">
        <view class="result-card">
          ...
          <view class="details-box">
            ...
            <view class="detail-item">
              <text class="detail-label">音频时长：</text>
              <text class="detail-value">{{ detectionResult.duration }}秒</text>
            </view>
            <view class="detail-item">
              <text class="detail-label">采样率：</text>
              <text class="detail-value">{{ detectionResult.sampleRate }} Hz</text>
            </view>
          </view>
    ...
    <!-- 功能说明 -->
    <view class="info-section" v-if="!audioUrl && !detectionResult">
      ...
      <view class="info-list">
        <view class="info-item">
          ...
          <view class="info-content">
            <text class="info-name">声纹分析</text>
            <text class="info-desc">检测AI合成声音特征</text>
          </view>
        </view>
        <view class="info-item">
          <text class="info-icon">🔊</text>
          <view class="info-content">
            <text class="info-name">频谱检测</text>
            <text class="info-desc">分析音频频谱异常</text>
          </view>
        </view>
        <view class="info-item">
          <text class="info-icon">🎯</text>
          <view class="info-content">
            <text class="info-name">高精度识别</text>
            <text class="info-desc">基于深度学习模型</text>
          </view>
        </view>
      </view>
    </view>
...
export default {
  ...
  methods: {
    ...
    // 选择音频
    chooseAudio() {
      // uni-app 没有直接的 chooseAudio API，需要使用 chooseMessageFile 或其他方式
    ...
    startDetection() {
      ...
      // 使用统一的http工具进行上传
      http.upload('/ai/detection/audio/upload', this.tempFilePath)
        .then(data => {
          if (data.code === 200) {
            const record = data.data
            // 解析检测详情
            const details = JSON.parse(record.detectionDetails || '{}')
            ...
            this.detectionResult = {
              ...
              sampleRate: details.details?.sampleRate || 44100,
    ...
    getResultTitle() {
      if (!this.detectionResult) return ''
      return this.detectionResult.isAI ? 'AI合成音频' : '真实录音音频'
    },

    // 获取结果文本
    getResultText() {
      if (!this.detectionResult) return ''
      return this.detectionResult.isAI ? 'AI合成' : '真实录音'
    },
